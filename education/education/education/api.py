...
def before_user_insert(doc, method=None):
	if getattr(doc, "custom_approval_status", None) == "Pending":
		doc.enabled = 0

def on_user_update(doc, method=None):
	if doc.has_value_changed("custom_approval_status"):
		if doc.custom_approval_status == "Approved":
			doc.enabled = 1
		elif doc.custom_approval_status == "Rejected":
			doc.enabled = 0
